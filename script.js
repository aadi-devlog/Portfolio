...
const projects = [
  ...
  },
  {
    ...
    blogIntro: 'This project originated as part of the FUME GenAI Product Intern assignment — a challenge that asked candidates to design and build a Client Intelligence Platform capable of transforming coach-client conversations into structured, reliable insights. The assignment was not simply asking for an AI application. It required demonstrating real product thinking: how do you make AI output trustworthy enough to be used in a professional setting where a wrong interpretation of a client conversation could lead to a real mistake? That question became the foundation of every design decision. The initial goal was to prove that AI could reliably analyze a coaching transcript and surface meaningful insights without inventing information that was not actually said. During development, the project grew beyond the original assignment scope. Improvements were made to the validation layer, the review workflow was redesigned to make it easier for non-technical users to interact with, the AI analysis was made more consistent across different transcript formats, and additional safeguards were added to catch contradictions in the AI output before a report reached a reviewer. The result is a platform that reads less like a hackathon submission and more like an early-stage product built with the kind of care you would expect from a professional engineering team.',
    highlights: [
